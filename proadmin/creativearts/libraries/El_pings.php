...
/**
 * Creativearts Creativelab Pings Class
 *
 * @package		Creativearts
...
class El_pings {
	...
	public function is_registered()
	{
		...
		if ( ! $cached OR $cached != $exp_response)
		{
			...
			if ($class == 'homepage' OR ($class == 'admin_system' && $method == 'software_registration'))
			{
				...
				if ( ! $registration = $this->_do_ping('http://ping.creativelab.com/register.php', $payload))
				{
	 ...
	 *
	 * Checks the current version of Creativearts available from Creativelab
	 *
	 * @access	private
	 * @return	string
	 */
	public function get_version_info()
	{
		...
		if ( ! $cached)
		{
			...
			if ( ! $version_info = $this->_do_ping('http://versions.creativelab.com/versions_ee2.txt'))
			{
	...
	private function _do_ping($url, $payload = null)
	{
		...
		if ( ! empty($payload))
		{
			...
			fputs($fp, "User-Agent: EE/Creativelab PHP/\r\n");
...
/* Location: ./system/creativearts/libraries/El_pings.php */
