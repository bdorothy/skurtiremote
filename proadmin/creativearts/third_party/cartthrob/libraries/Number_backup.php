...
class Number
{
	...
	**/
	public function format($number)
	{
		...
		/// NEW MANIPULATIONS BEGIN FROM HERE :
		ee()->load->library('ls_currency_calculator');
		$number = ee()->ls_currency_calculator->converted_numeric($number,"");
		...
		$formatted_number = number_format($number, $this->decimals, $this->dec_point, $this->thousands_sep);
		...
		{
			$number = $formatted_number." ".$prefix;
		}
		else
		{
			$number = $prefix.$space.$formatted_number;
		}
