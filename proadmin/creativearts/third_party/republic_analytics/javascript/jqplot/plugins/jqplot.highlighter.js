...
(function($) {
    ...
    function showTooltip(plot, series, neighbor) {
        ...
        if (hl.useAxesFormatters) {
            ...
            var str;
            xfstr = '%Y-%m-%d';
            var xstr = xf(xfstr, neighbor.data[0]); // Använda
        ...
        // Republic
        var pageViews = "";
        if (plot.series[2] !== undefined) {
        	pageViews = plot.series[2].label + ': ' + plot.series[2]._plotData[neighbor.pointIndex][1];
        }
        var visits = plot.series[1].label + ': ' + plot.series[1]._plotData[neighbor.pointIndex][1];
        var visitors = plot.series[0].label + ': ' + plot.series[0]._plotData[neighbor.pointIndex][1];
        var colors = plot.colors;

        var elementHtml = '<div class="date">' + xstr + '</div>' + visits + '<br />' + visitors;
        if (pageViews !== "") {
        	elementHtml = elementHtml + '<br />' + pageViews;
        }
    ...
    function handleMove(ev, gridpos, datapos, neighbor, plot) {
        ...
        // Republic
        if(neighbor != null && neighbor.pointIndex != old_neighbor) {
          hl.isHighlighting = false;
          old_neighbor = neighbor.pointIndex;
        }
