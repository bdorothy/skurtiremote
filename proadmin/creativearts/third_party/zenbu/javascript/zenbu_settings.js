$(document).ready(function () {
	...
	*/
	...
	/**
	 * ========================================================
	 * Display Settings collapsing of unshown rows
	 * ========================================================
	 */
	$("button.collapsetoggle").click( function () {

		if( $(this).find("span").eq(0).hasClass("invisible") )
		{
			$(this).find("span").removeClass("invisible").eq(1).addClass("invisible");
			collapsedStatus = 1;
		} else {
			$(this).find("span").removeClass("invisible").eq(0).addClass("invisible");
			collapsedStatus = 0;
    	}
		...
		{
			// ---------------------------------
    		// Put rows into invisible temporary stash
    		// ---------------------------------
 			
			$("table.settingsTable tbody tr").each(function()
			{
			    if( $(this).children("td.display").children("input:checked").length == 0)
			    {
			    	if( $(this).has($("td.display")) )
			    	{
			    		rowToMove = $(this).clone();
			    		$(this).remove();
			    		$("div.collapsedrows").append(rowToMove);   		
			    	}
			    }	
			});
			...
			// ---------------------------------
			// Put stashed rows back into table, 
			// and empty the stash for next use
			// ---------------------------------
	    	
	    	rowsToPutBack = $("div.collapsedrows").html();
	    	$("table.settingsTable tbody").append(rowsToPutBack);
	    	$("div.collapsedrows").empty();
		}
