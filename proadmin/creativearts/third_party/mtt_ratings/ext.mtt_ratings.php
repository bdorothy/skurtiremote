...
class Mtt_ratings_ext {

	
	public $settings 		= array();
	public $description		= 'MyTopTeacher - Ratings Extension';
	public $docs_url		= 'http://www.sixth.co.in';
	public $name			= 'MTT Ratings';
	...
	$knowledge = ee()->input->post('knowledge');
	$communication = ee()->input->post('communication');
	$attention = ee()->input->post('attention');
	$patience = ee()->input->post('patience');
	$fees = ee()->input->post('fees');
	$parent_id = ee()->input->post('PID');
	
	// Get the amount and per session
	$amount = ee()->input->post('amount');
	$session = ee()->input->post('session');
	
	// Does the user want to be anonymous
	$anonymous = ee()->input->post('anonymous');
	...
	ee()->db->insert('mtt_ratings', $data); 
	$this->update_ratings_stats($entry_id);
	
	// do not throw ajax for rating box
	if($parent_id != 0){
	return $this->EE->output->send_ajax_response(array('success' => true, 'field_errors' => 'We have recieved your response. We shall publish the same once approved by our administrators'));
	}
	}
	
	
	public function update_ratings_stats($entry_id){
	// records
	$records = array(
	'entry_id'					=> $entry_id,
	'overall_ratings'			=> $this->overall_ratings($entry_id),
	'total_ratings'				=> $this->total_ratings($entry_id),
	);
	
	ee()->db->where('entry_id',$entry_id);
	if (ee()->db->count_all_results('exp_mtt_ratings_stats') == 0) {
    $query = ee()->db->insert('exp_mtt_ratings_stats', $records);
    } else {
      // A record does exist, update it.
    $query = ee()->db->update('exp_mtt_ratings_stats', $records, array('entry_id'=>$entry_id));
    }
