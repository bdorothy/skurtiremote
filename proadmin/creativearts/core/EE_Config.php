...
class EE_Config Extends CI_Config {
	...
	function _initialize()
	{
		...
		// Is the config file blank?  If so it means that Creativearts has not been installed yet
		if ( ! isset($config) OR count($config) == 0)
		{
			// If the admin file is not found we show an error
			show_error('Creativearts does not appear to be installed.  If you are accessing this page for the first time, please consult the user guide for installation instructions.', 503);
		}
	...
	function _update_dbconfig($dbconfig = array(), $remove_values = array())
	{
		...
		{
			show_error('Your database.php file does not appear to have the proper file permissions.  Please set the file permissions to 666 on the following file: creativearts/config/database.php', 503);
		}
...
/* Location: ./system/creativearts/libraries/EE_Config.php */
