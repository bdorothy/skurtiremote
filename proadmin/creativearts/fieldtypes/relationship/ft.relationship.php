...
class Relationship_ft extends EE_Fieldtype {
	...
	public function display_field($data)
	{
		...
		ee()->db->distinct();
		ee()->db->select('channel_titles.entry_id, channel_titles.title, channel_data.field_id_8 as sku, channel_data.field_id_2 as price');
		ee()->db->from('channel_titles');
		ee()->db->join('channel_data', 'channel_titles.entry_id = channel_data.entry_id');
		ee()->db->order_by($order_field, $this->settings['order_dir']);
